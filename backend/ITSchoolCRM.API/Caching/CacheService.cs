...
/// (после старта приложения — abortConnect=false страхует только
/// старт) каждый метод перехватывает исключение, пишет warning
/// в лог и обслуживает запрос напрямую из источника (PostgreSQL).
...
public sealed class CacheService : ICacheService
{
    private static readonly JsonSerializerOptions JsonOptions = new()
    {
        ...
        // null-поля не пишем: короче payload, десериализация
        // вернёт null и без них
        ...
        CancellationToken ct = default)
    {
        ...
        // Шаг 3: промах — загружаем из источника (PostgreSQL
        // через EF Core). Factory вызываем ДО записи в кэш:
        // если источник бросил исключение, в кэш не попадёт
        // «пустышка». Исключение источника НЕ перехватываем —
        // оно реально означает сбой, его видит глобальный
        // обработчик (ERR_INTERNAL).
        var value = await factory(ct);

        // Шаг 4: запись в кэш — best effort. Неудача записи
        // не ломает запрос: данные получены из источника,
        // просто следующий запрос снова сделает промах.
        try
        {
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(
                ...
                "Результат возвращён без кэширования.",
                resolvedKey);
        }
    ...
    public async Task BumpVersionAsync(
        ...
        CancellationToken ct = default)
    {
        // Инвалидация — best effort: если KeyDB лежит, TTL
        // всё равно обновит протухшие записи, а прерывать
        // запись из-за сброса кэша недопустимо.
        try
        {
            ...
            var newVersion = await db.StringIncrementAsync(
                versionKey);

            _logger.LogDebug(
                "Cache version bumped: {VersionKey} -> v{Version}",
                versionKey,
                newVersion);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(
                ...
                "Инвалидация пропущена (TTL обновит записи).",
                versionKey);
        }
    }
        ...
        CancellationToken ct = default)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(
                ex,
                "KeyDB недоступна при удалении ключа {Key}.",
                key);
        }
    }
    ...
    ///
    /// Пример: "interaction:5:v{interaction:version}" при счётчике 7
    /// превращается в "interaction:5:v7".
    ///
    /// ВАЖНО: метод НЕ перехватывает исключения KeyDB — это делает
    /// вызывающий GetOrCreateAsync, чтобы решить о fallback.
    /// </summary>
    private async Task<string> ResolveVersionsAsync(
        string key,
        CancellationToken ct)
    {
        ...
        while (true)
        {
            ...
            if (end < 0)
            {
                ...
                _logger.LogWarning(
                    "Незакрытый заполнитель версии в ключе кэша: {Key}",
                    key);
                break;
            }

            var versionKey = result.Substring(
                start + 1,
                end - start - 1);

            var version = await db.StringGetAsync(
                versionKey);

            var versionValue = version.HasValue
                ? (string)version!
                : "1";

            result = result
                .Remove(start, end - start + 1)
                .Insert(start, versionValue);
        }
