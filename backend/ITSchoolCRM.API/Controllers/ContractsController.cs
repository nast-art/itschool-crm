...
public class ContractsController : ControllerBase
{
    ...
    public async Task<ActionResult<List<ContractDto>>> GetAll(CancellationToken cancellationToken)
    {
        var contracts = await _service.GetAllAsync(cancellationToken);
    ...
    [Authorize(Policy = Policies.UserAccess)]
    public async Task<ActionResult<List<ContractInteractionOptionDto>>> GetInteractionOptions(CancellationToken cancellationToken)
    {
        var options = await _service.GetInteractionOptionsAsync(cancellationToken);
    ...
    /// POST возвращает созданный DTO (тот же тип, что у GET) — единообразие
    /// типов ответа упрощает Swagger-документацию.
    /// </summary>
    /// <remarks>
    ...
    public async Task<ActionResult<ContractDto>> Create([FromBody] ContractCreateDto dto, CancellationToken cancellationToken)
    {
        var contract = await _service.CreateAsync(dto, cancellationToken);
    ...
    [HttpPut("{id:int}")]
    [Authorize(Policy = Policies.ManagerAccess)]
    public async Task<ActionResult<ContractDto>> Update(int id, [FromBody] ContractUpdateDto dto, CancellationToken cancellationToken)
    {
