...
public class AttachmentsController : ControllerBase
{
    ...
    [HttpPost(
        "interaction/{interactionId:int}/status/{statusId:int}")]
    ...
    [HttpGet(
        "interaction/{interactionId:int}")]
    [Authorize(Policy = Policies.UserAccess)]
    public async Task<
        ActionResult<List<AttachmentDto>>> GetByInteraction(
        int interactionId,
        CancellationToken cancellationToken)
    {
        var attachments =
            await _service.GetByInteractionAsync(
                interactionId,
                cancellationToken);
    ...
    [HttpGet("{id:int}")]
    [Authorize(Policy = Policies.UserAccess)]
    public async Task<ActionResult<AttachmentDto>> GetById(
        int id,
        CancellationToken cancellationToken)
    {
        var attachment =
