...
public class WebsiteMockController : ControllerBase
{
    ...
    // Принятые взаимодействия: List + lock — простой вариант
    // для заглушки; статика, т.к. контроллер не хранит состояния.
    private static readonly List<PushInteractionToWebsiteDto> ReceivedInteractions = new();

    private static readonly object ReceivedInteractionsLock = new();

    private readonly IConfiguration _configuration;
    private readonly ILogger<WebsiteMockController> _logger;

    public WebsiteMockController(IConfiguration configuration, ILogger<WebsiteMockController> logger)
    {
        _configuration = configuration;
        _logger = logger;
    }

    // 401 с единым контрактом { code, message } — одинаковый ответ
    // в эндпоинтах свёрнут сюда.
    ...
    public ActionResult ReceiveInteraction([FromBody] PushInteractionToWebsiteDto dto)
    {
        if (!IsAuthorized())
        {
            return UnauthorizedResponse();
    ...
    // Ключ не задан в конфигурации — считаем мок открытым (удобно для
    // локальной разработки; в стенде ключ задаётся обязательно).
    private bool IsAuthorized()
    {
        var expectedKey = _configuration["Integrations:MockApiKey"];
