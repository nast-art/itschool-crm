...
namespace ITSchoolCRM.API.Integrations.LMS.Services
{
    ...
    public class MockLmsIntegrationService : ILmsIntegrationService
    {
        ...
        public MockLmsIntegrationService(
            IHttpClientFactory httpClientFactory,
            IOptions<LmsIntegrationOptions> options,
            ILogger<MockLmsIntegrationService> logger)
        {
        ...
        public async Task<List<LmsStudentDto>> GetStudentsAsync(
            CancellationToken cancellationToken)
        {
            var students =
                await SendAsync<List<LmsStudentDto>>(
                    HttpMethod.Get,
                    "api/mock-lms/students",
                    payload: null,
                    cancellationToken);
        ...
        public async Task<List<LmsCourseDto>> GetCoursesAsync(
            CancellationToken cancellationToken)
        {
            var courses =
                await SendAsync<List<LmsCourseDto>>(
                    HttpMethod.Get,
                    "api/mock-lms/courses",
                    payload: null,
                    cancellationToken);
        ...
        public async Task<List<LmsEnrollmentDto>> GetEnrollmentsAsync(
            CancellationToken cancellationToken)
        {
            var enrollments =
                await SendAsync<List<LmsEnrollmentDto>>(
                    HttpMethod.Get,
                    "api/mock-lms/enrollments",
                    payload: null,
                    cancellationToken);
        ...
        public async Task PushInteractionAsync(
            PushInteractionToLmsDto dto,
            CancellationToken cancellationToken)
        {
            await SendAsync<object>(
                HttpMethod.Post,
                "api/mock-lms/interactions",
                payload: dto,
                cancellationToken);
        }
            ...
            while (true)
            {
                        ...
                        or IOException)
                {
                    if (attempt >= _options.RetryCount)
                    {
                        ...
                        throw new IntegrationException(
                            IntegrationErrorCodes.LmsUnavailable,
                            "Внешняя LMS временно недоступна.",
                            ex);
                    }

                    _logger.LogWarning(
                        "Попытка {Attempt} обращения к LMS завершилась ошибкой. Повтор через {Delay} мс.",
                        attempt,
        ...
        private void EnsureEnabled()
        {
            ...
            {
                throw new IntegrationException(
                    IntegrationErrorCodes.ConfigurationInvalid,
