...
public class LmsMockController : ControllerBase
{
    ...
    private static readonly List<PushInteractionToLmsDto> ReceivedInteractions = new();
    ...
    // 401 с единым контрактом { code, message } — пять одинаковых
    // ответов в эндпоинтах свёрнуты сюда.
    ...
    public ActionResult ReceiveInteraction([FromBody] PushInteractionToLmsDto dto)
    {
        if (!IsAuthorized())
        {
            return UnauthorizedResponse();
