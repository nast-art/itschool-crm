...
///     catalog:directions:v{catalog:version}. Выборки НЕ
///     фильтруются по доступу (направления видят все роли,
///     п. 11 ТЗ) — поэтому скоп пользователя в ключе не нужен.
///     Исключение из скопа здесь осознанное, а не упущение:
...
///     каждую клавишу, каждая буква — новый одноразовый ключ.
///
/// ИНВАЛИДАЦИЯ: Create/Update/Delete после SaveChangesAsync делают
/// два INCR:
///   - catalog:version — сами справочники направлений;
///   - interaction:version — взаимодействия и статистика:
///     имя направления проецируется в InteractionDto через
///     program.direction и агрегируется в ReportService.ByDirection,
///     а приоритет (ранжирование, п. 4 ТЗ) влияет на порядок
///     выдачи. Счётчики обновляются за миллисекунды.
/// </summary>
public class DirectionService : IDirectionService
{
    ...
    /// </summary>
    private async Task InvalidateDirectionsAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        // общий кэш на всех. Промах → загрузка из PostgreSQL,
        // результат кладётся в KeyDB с TTL CatalogTtl (600 сек).
        return await _cache.GetOrCreateAsync(
            CacheKeys.Catalog(CacheKeys.Directions),
            _cacheOptions.CatalogTtl,
            _ => Project(
                    ApplyOrder(
                        _context.it_directions.AsNoTracking()))
    ...
    public async Task<List<DirectionDto>> SearchAsync(
        ...
        CancellationToken cancellationToken)
    {
        // БЕЗ КЭША ОСОЗНАННО: каждая клавиша в поиске — новый
        // ключ, все они протухнут неиспользованными. Кэш
        // одноразовых значений — это утечка памяти KeyDB.
        ...
        CancellationToken cancellationToken)
    {
        ...
        // НОВОЕ (кэш): направление появилось в справочниках и в
        // выборках программ (фронт группирует по направлениям).
        // Возвращаемый DTO собираем через GetByIdAsync — один
        // запрос вместо повторной проекции, в кэш он не попадёт.
        ...
        CancellationToken cancellationToken)
    {
        ...
        // НОВОЕ (кэш): имя, активность или ПРИОРИТЕТ (порядок
        // ранжирования карточек на фронте) изменились — справочник
        // и все производные выборки устарели.
        await InvalidateDirectionsAsync(cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        // НОВОЕ (кэш): направление удалено — справочник и выборки
        // устарели.
        await InvalidateDirectionsAsync(cancellationToken);
