...
///
/// ИНВАЛИДАЦИЯ: каждый метод записи (Create, ChangeStatus, Update,
/// AddComment) после SaveChangesAsync делает INCR счётчика
/// interaction:version — все версионированные ключи (карточки,
/// списки, история, вложения, статистика) перестают находиться
...
public class InteractionService : IInteractionService
{
    ...
    /// <summary>
    /// Скоп ключа кэша для текущего пользователя.
    /// manager/admin (полный доступ) — общий скоп "full";
    /// менеджер по вузам — свой собственный (KeycloakUserId).
    /// KeycloakUserId берём, а не БД-id пользователя: он гарантированно
    /// есть в JWT, тогда как users_id появляется после синхронизации.
    /// null — пользователь не аутентифицирован (не должно случаться:
    /// контроллер под [Authorize], но защищаемся).
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Нет идентификатора — кэшировать нечего
