...
namespace ITSchoolCRM.API.Services.Implementations
{
    public sealed class KeycloakOptions
    {
        ...
        /// <summary>Базовый URL Keycloak, например http://localhost:8080.</summary>
    ...
    public sealed class KeycloakAdminClient : IKeycloakAdminClient
    {
        ...
        /// <inheritdoc />
        public async Task<IReadOnlyList<KeycloakUserRecord>> ListUsersWithRolesAsync(
            CancellationToken ct)
        {
            ...
            var users = await usersResponse.Content
                .ReadFromJsonAsync<List<KcUserBrief>>(ct)
                ?? [];
            ...
            foreach (var role in AppRoles)
            {
                ...
                var members = await roleResponse.Content
                    .ReadFromJsonAsync<List<KcUserBrief>>(ct) ?? [];
            ...
            string keycloakUserId, string newRole, CancellationToken ct)
        {
            ...
            foreach (var role in AppRoles)
            {
                ...
                var rep = await getResponse.Content
                    .ReadFromJsonAsync<KcRoleRepresentation>(ct)
            ...
            var mappingsPath =
                $"/admin/realms/{_opt.Realm}/users/{keycloakUserId}/role-mappings/realm";

            if (toRemove.Count > 0)
            {
                using var removeRequest = await CreateRequestAsync(
                    HttpMethod.Delete, mappingsPath, ct);
            ...
            if (toAdd.Count > 0)
            {
                using var addRequest = await CreateRequestAsync(
                    HttpMethod.Post, mappingsPath, ct);
        ...
        // ИСПРАВЛЕНО: была static — не имела доступа к _logger.
        // Теперь метод экземпляра, детали ошибки уходят в лог,
        // в исключение попадает краткое сообщение для фронта.
